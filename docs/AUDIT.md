...
Анализ проводится по критериям оценки из `docs/task.md`.

### 1. Базовая работоспособность (макс. 20 баллов)

| Требование | Статус | Комментарий |
|-----------|--------|-------------|
| Обнаружение узлов | ✅ Реализовано | mDNS (zeroconf) + UDP broadcast на порту 47821 |
| Установка P2P-сессии | ✅ Реализовано | TCP соединения с ECDH handshake |
| Обмен текстовыми сообщениями | ✅ Реализовано | Глобальный чат + прямые зашифрованные сообщения |
| **Мультихоп-цепочка** | **❌ Отсутствует** | **Только прямые point-to-point соединения. Нет relay/forwarding. Ни реализации, ни описания архитектуры в коде нет** |

**Потенциальные потери:** до 5 баллов за отсутствие мультихопа. ТЗ допускает описание протокола/архитектуры как альтернативу, но в репозитории нет ни того, ни другого.

---

### 2. Архитектура сети (макс. 20 баллов)

| Требование | Статус | Комментарий |
|-----------|--------|-------------|
| Топология и логика появления/ухода узлов | ✅ Реализовано | `PeerManager`, события `PeerJoined`/`PeerLeft`, reconnect через discovery |
| **Алгоритм маршрутизации** | **❌ Отсутствует** | **Нет маршрутизации вообще — только flat P2P. Все узлы должны видеть друг друга напрямую** |
| **Дедупликация сообщений** | **❌ Частично** | **Дедупликация есть только для call-сигналов (`_seenSignals`). Для текстовых и файловых сообщений дедупликации нет совсем** |
| **Защита от петель** | **❌ Отсутствует** | **При отсутствии мультихопа петли невозможны, но защита не реализована даже в потенциальной архитектуре** |
| Работа при разрывах | ⚠️ Частично | Discovery переоткрывает соединения, но очереди недоставленных сообщений нет — сообщения теряются при разрыве |

**Потенциальные потери:** 6–10 баллов. Отсутствие алгоритма маршрутизации и дедупликации — ключевые пункты критерия.

---

### 3. Real-time звонки (макс. 20 баллов)

| Требование | Статус | Комментарий |
|-----------|--------|-------------|
| Транспортный протокол и буферы | ⚠️ Частично | WebRTC реализован, но параметры джиттер-буфера, стратегия потерь, таймауты не описаны и не настроены явно |
| **Методика фактических замеров** | **❌ Отсутствует** | **Нет кода для измерения задержки, джиттера, потерь. Нет документации методики измерений. Метрики `CallService._counters` — только счётчики звонков, не качества** |
| Стабильность при потерях | ⚠️ Частично | WebRTC обеспечивает базовую стабильность, но нет ICE restart при деградации соединения |
| Интерактивность UI при потерях | ✅ Реализовано | UI показывает состояние звонка через `CallState`, таймер длительности |

**Потенциальные потери:** 6 баллов за отсутствие методики замеров — это обязательный пункт ТЗ.

---

### 4. Надёжность (макс. 15 баллов)

| Требование | Статус | Комментарий |
|-----------|--------|-------------|
| **Гарантия доставки сообщений** | **❌ Отсутствует** | **Нет Ack для текстовых и файловых сообщений. Fire-and-forget. Сообщения при разрыве соединения теряются без уведомления** |
| Очереди / ретраи / Ack | ⚠️ Частично | Ack+retry реализован только для call-сигналов (`_pendingAcks`, `_ackBackoff`). Для сообщений и файловых чанков — нет |
| **Контроль перегрузки** | **❌ Отсутствует** | **Нет rate limiting, нет backpressure, нет QoS. Передача большого файла может монополизировать TCP-соединение и задержать сообщения и call-сигналы** |
| **Тесты на отказ** | **❌ Отсутствует** | **В репозитории есть только дефолтный `flutter-app/test/widget_test.dart` без реального содержания. Нет ни одного теста на отказ, смену сети или отключение пира** |

**Потенциальные потери:** 8–12 баллов. Отсутствие тестов и контроля перегрузки — прямые требования ТЗ.

---

### 5. Передача файлов (макс. 10 баллов)

| Требование | Статус | Комментарий |
|-----------|--------|-------------|
| Протокол (чанки, хэш, контроль целостности) | ✅ Реализовано | 512 KB чанки, SHA-256 на всём файле, проверка хэша при `FileDone` |
| **Возобновление передачи / частичная доставка** | **❌ Отсутствует** | **При разрыве соединения передача начинается заново. Нет сохранения позиции, нет частичного файла с возможностью дозагрузки** |
| **Ограничение нагрузки** | **❌ Отсутствует** | **Нет rate limiting для чанков, нет ограничения параллельных передач, нет приоритизации трафика (сообщения vs файл). Чанки шлются с `time.Sleep(0)` — без задержки** |

**Потенциальные потери:** 3–6 баллов. Возобновление и ограничение нагрузки — прямые требования раздела 6 ТЗ.

---

### 6. Документация и тестируемость (макс. 5 баллов)

| Требование | Статус | Комментарий |
|-----------|--------|-------------|
| **README** | **❌ Отсутствует** | **Корневого README.md нет. `flutter-app/README.md` — стандартный Flutter-шаблон без описания проекта** |
| **Архитектурная схема** | **❌ Отсутствует** | **Нет ни диаграммы, ни схемы в `docs/`. Архитектура описана только в комментариях к коду** |
| Логи / метрики | ✅ Реализовано | `piper.log` (TUI), `LogService` (Flutter), `CallService._counters` |

**Потенциальные потери:** до 4 баллов.

---

### 7. Безопасность (макс. 10 баллов)

| Требование | Статус | Комментарий |
|-----------|--------|-------------|
| Шифрование | ✅ Реализовано | X25519 ECDH + ChaCha20-Poly1305 для DM и групповых чатов |
| **Аутентификация и идентификация узлов** | **❌ Не выполнено** | **PeerID самовыбирается и не верифицируется. Нет подписи, нет PKI. MITM возможен (см. C-2)** |
| **Защита от спама и подмены** | **❌ Не выполнено** | **Нет rate limiting на сообщения. Спуфинг в глобальном чате (см. C-3). Нет модели угроз в документации** |
| Глобальный чат — plaintext | ⚠️ Архитектурное решение | Явно обозначено в коде, но не задокументировано для пользователя |

**Потенциальные потери:** 4–6 баллов. Аутентификация и защита от подмены — прямые требования ТЗ.

---

### Бонусная часть (макс. 60 баллов)

| Критерий | Статус | Комментарий |
|---------|--------|-------------|
| NAT, Wi-Fi Direct, BLE fallback | ❌ | Только LAN (mDNS + UDP broadcast). Нет STUN/TURN, нет Wi-Fi Direct, нет BLE |
| Групповые чаты | ✅ | Групповые чаты с E2E шифрованием реализованы |
| Доказанная оптимизация vs аналоги | ❌ | Нет сравнительных бенчмарков, нет описания оптимизаций |
| Кросс-платформенность | ✅ | Android (arm64, x86_64) + Windows (TUI + Flutter) |
| Красивый UI / удобный UX | ✅ | Flutter UI с тёмной темой, полноценные экраны |
| **Написанные тесты** | **❌** | **Только пустой `widget_test.dart`. Ни unit, ни integration тестов** |

---

### Итоговая оценка рисков по ТЗ

| Раздел ТЗ | Макс. | Риск потерь | Основные пробелы |
|----------|-------|-------------|-----------------|
| 1. Базовая работоспособность | 20 | ~5 | Мультихоп не реализован |
| 2. Архитектура сети | 20 | ~10 | Нет маршрутизации, нет дедупликации сообщений |
| 3. Real-time звонки | 20 | ~6 | Нет методики замеров |
| 4. Надёжность | 15 | ~10 | Нет Ack для сообщений, нет тестов отказа |
| 5. Передача файлов | 10 | ~6 | Нет resume, нет rate limiting |
| 6. Документация | 5 | ~4 | Нет README и архитектурной схемы |
| 7. Безопасность | 10 | ~6 | Нет аутентификации пиров |
| **Итого базовая** | **100** | **~47** | |
| Бонус: тесты | 10 | ~10 | Тестов нет |
| Бонус: NAT/BLE | 10 | ~10 | Не реализовано |
| **Итого бонус** | **60** | **~20** | |

**Оценочный диапазон при текущем состоянии: ~50–70 / 160 баллов.**
