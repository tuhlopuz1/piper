...
// It also fetches messages from our own DHT inbox on startup.
...
// NewOfflineStore creates an OfflineStore.
func NewOfflineStore(keys identity.Keys, senderPeerID string, kadDHT *kaddht.IpfsDHT, db *bbolt.DB) *OfflineStore {
	return &OfflineStore{
		keys:         keys,
...
// msgID must be globally unique (a UUID).
func (s *OfflineStore) Store(ctx context.Context, msgID, text string, recipientX25519Pub [32]byte) error {
	...
	if err != nil {
		return fmt.Errorf("offline_store: sign payload: %w", err)
	}
...
// updateInbox fetches, merges, signs, and re-publishes the inbox at inboxKey.
func (s *OfflineStore) updateInbox(ctx context.Context, inboxKey string, newItem DHTInboxItem) error {
	// Best-effort: get existing inbox to merge.
	...
	// Prune expired; cap at maxInboxItems (evict oldest).
	now := time.Now()
...
// publishInbox signs and puts the given items to the DHT at inboxKey.
